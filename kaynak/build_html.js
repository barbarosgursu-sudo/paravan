...
/* ============================================================
   SES — yalnızca kısa efektler. ARKA PLAN MÜZİĞİ YOK.
   Sahibinin kararı (19 Eylül 2026): müzikli hâli beğenilmedi, oyun
   sessizlik + efekt üzerine kuruldu. Sekiz müzik parçası üretilip
   gömülmüştü; hepsi kaldırıldı. Gerekçe ve geri dönüş tarifi:
   kaynak/ses_promptlari.md → MÜZİK KALDIRILDI.
   Efektler GÖMÜLMEZ; ses/ klasöründen çalınır. Dosya yoksa oyun
   sessiz devam eder, hata vermez.
   ============================================================ */
...
function sesKesfet(){
  ...
  const adlar = [...new Set(Object.values(EFEKT))];
...
function efektCal(tur, i){
  ...
  try{
    ...
    a.currentTime = 0;
    // Eskiden müziğin bir tık üstünde dursun diye +0.15 eklenirdi; müzik
    // kaldırılınca üstünde duracağı bir şey kalmadı. Artık efektler tam
    // olarak kullanıcının seçtiği düzeyde çalıyor.
    a.volume = ses.seviye;
...
function devPanel(){
  ...
  h += \`<div class="dev-blok"><div class="dev-baslik">Ses teşhisi</div>
    <div class="dev-satir">açık: <b>\${ses.acik}</b> · kilitli: <b>\${ses.kilitli}</b></div>
...
function devSesDene(){
  ...
  const a = new Audio(SES_KLASOR + EFEKT.muhur + SES_UZANTILAR[ses.uzanti[EFEKT.muhur] ?? 0]);
  a.volume = 1;
  const p = a.play();
...
function devSesRapor(){
  const r = {
    adres: location.href,
    acik: ses.acik, kilitli: ses.kilitli,
...
/* Cengo–Peri ikili kareleri. RUH_GORSEL'in kardeşi: dört görsel, cengoDurum()'un
   ...
   DOSYA ADLARI KADEME ADLARIYLA EŞLEŞMEZ — bilerek. 22 Eylül 2026'da dört kare
   yan yana konunca cengo_yoldas ile cengo_yakin'in SICAKLIĞININ TERS olduğu
   görüldü: cengo_yoldas ikisinin gülümsediği en sıcak kare, cengo_yakin ise
   sessiz ve gülümsemesiz (dosyanın kendi alt metni de "kimse konuşmuyor" diyordu).
   Yoldaş kademesi bağ -1..+2 aralığını, yani oyunun çoğunu kaplar; taban gülümserse
   üstündeki iki kademenin gidecek yeri kalmaz ve soğuk kararlar gülümseyen kareyle
   çıkar. Kareler burada takas edildi; dosyalar yeniden üretilmedi, sayım 63'te kaldı.
   Merdiven artık tek yönde yükseliyor: sırtı dönük -> sessiz bakışma -> gülümseme ->
   uyuyakalma. Dosya adını kademeye uydurmak için buradaki eşleşmeyi geri çevirme. */
const CENGO_GORSEL = {
  "Mesafeli": { dosya: 'cengo_mesafeli.jpg', alt: 'Cengo paltosuyla kapıya yakın, Peri masada; aralarında odanın boyu' },
  "Yoldaş":   { dosya: 'cengo_yakin.jpg',    alt: 'İkisi karşılıklı oturmuş, bakışıyor, kimse konuşmuyor' },
  "Yakın":    { dosya: 'cengo_yoldas.jpg',   alt: 'Cengo masanın köşesinde, ikisi bakışıyor' },
